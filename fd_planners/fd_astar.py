...
def run_fastdownward(timeout: int, eval: str, domain: str, problem: str, output: str) -> None:
    ...
    command = f'python3 ../downward/fast-downward.py --plan-file {output} --overall-time-limit {timeout}s --overall-memory-limit 4G --sas-file {sas_filename} {domain} {problem} --evaluator "h={eval}()" --search "astar(h())"'
